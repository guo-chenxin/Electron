...
const initializeTabState = () => {
  ...
  const savedPaths = localStorage.getItem('lastVisitedPaths');
...
export function useTabs() {
  ...
  const saveTabState = () => {
    ...
    localStorage.setItem('lastVisitedPaths', JSON.stringify(lastVisitedPaths.value));
  };

  // 从数据库获取路由数据并更新标签页配置
  const fetchRoutesForTabsLocal = async () => {
    try {
      // 从主进程获取路由配置
      const routesFromDb = await window.electronAPI.invoke<any[]>('api:routes:getAllNested');
